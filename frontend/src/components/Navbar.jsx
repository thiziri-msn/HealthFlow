...
const Navbar = () => {
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
            {/* Logo */}
            <h1 onClick={() => navigate('/')} className="text-2xl font-bold text-blue-900 cursor-pointer ">HealthFlow </h1>
            ...
            <ul className='hidden md:flex items-start gap-5 font-medium'>
                ...
                <NavLink to='/doctors' >
                    <li className='py-1'>NOS MÉDECINS</li>
                    <hr className='border-none outline-none h-0.5 bg-prim w-3/5 m-auto hidden' />
                </NavLink>
                <NavLink to='/about' >
            ...
            <div className='flex items-center gap-4'>
                {
                    token && userData
                        ...
                        : <button onClick={() => navigate('/login')} className='bg-prim text-white px-8 py-3 rounded-full
                    font-light hover:bg-blue-700 hidden md:block'>S'inscrire</button>
                }
                ...
                <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    ...
                    <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                        ...
                        <NavLink onClick={() => setShowMenu(false)} to={'/doctors'}><p className='px-4 py-2 rounded inline-block'>NOS MÉDECINS</p></NavLink>
